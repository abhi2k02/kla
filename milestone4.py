...
def findPoly(p1, p2):
    newpoly = []
    for k in range(2):
        area = polygonArea(p1[k])
        for i in p2:
            if p1[k]['n'] == i['n'] and p1[k]['layer'] == i['layer']:
                newpoly.append(i)
                s1 = math.dist(p1[k]['xy'][0], p1[k]['xy'][1])
                ...
                try:
                    ...
                    continue
                j = 0
                for j in range(len(p1[k]['xy'])-1):
                    s1 = math.dist(p1[k]['xy'][j], p1[k]['xy'][j+1])
